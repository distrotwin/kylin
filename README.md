...
所以它适合回答「编出来的东西对不对」，不适合回答「跑起来的系统对不对」。镜像与厂商安装介质之间还有一道包版本上的缝，量到底有多大见[镜像等于公开归档，不等于安装介质](#镜像等于公开归档不等于安装介质)。
...
V10 SP1 另配了两个更新源 `10.1-2403-updates` 与 `10.1-2203-updates`（`EXTRA_SUITES`），理由见上面[镜像等于公开归档，不等于安装介质](#镜像等于公开归档不等于安装介质)。
