...
针对这一点，V10 SP1 现在额外配了厂商自己文档里列的两个更新源 `10.1-2403-updates` 与 `10.1-2203-updates`，精确补齐 150 项差异里的 71 项，`ca-certificates` 推到 2023-03、`openssl` 与 `libc6` 同步跟进。四个版本里**只有 V10 SP1 有更新源**。V11、V4 各自的 `-updates` 候选全部 404（探测时带一条必然不存在的假 suite 做对照，它同样 404，说明这台主机确实区分真假路径、不是被拦），V11 的归档就是比介质旧半年，补不上。V10 原版不需要——它的 suite `10.0` 自述 `Label: v10-离在线更新推送`，本身就是滚动的更新通道，`Date` 停在 2025-03。
